export default class SetView {
  ...
  render() {
    ...
    const textScale = d3.scaleLog().domain(DE).range([8, 24]);
    ...
    subjectGroups
      ...
      .attr("font-size", (d) => `${textScale(d.value)}px`) // scale text based on prediction value
      .text((d) => d.name)
      ...
      if (!selected) {
        ...
        if (self.controls.sortBy === "rank") {
          ...
          subjectGroups
            ...
            .attr("font-size", (d) => `${textScale(d.value)}px`); // re-draw basic view
