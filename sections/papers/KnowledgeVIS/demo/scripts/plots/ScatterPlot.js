export default class ScatterPlot {
  ...
  render() {
    ...
    // compute scales
    const connectorLineStrokeWidthScale = d3.scaleLog().domain(DE).range([1, 4]);
    const connectorLineOpacityScale = d3.scaleLog().domain(DE).range([0.1, 0.9]);
    ...
    const predictionPoints = scatterPlot
      ...
      .attr("transform", (d) => `translate(${xScale(d.x) - 3},${yScale(d.y) - 3})`) // in px
