...
function saveData(data) {
  ...
  let subjectsFilter = [];
  if (groups.length == 1) {
    // 1 sentence group, 5 subjects at most
    subjectsFilter = groups
      .map((sentenceGroup) => sentenceGroup.subjects.map((subject) => subject.id).slice(0, 5))
      .flat();
  } else if (groups.length == 2) {
    // 2 sentence groups, 3 subjects at most
    subjectsFilter = groups
      .map((sentenceGroup) => sentenceGroup.subjects.map((subject) => subject.id).slice(0, 3))
      .flat();
  } else if (groups.length >= 3) {
    // 3+ sentence groups, 5 sentence groups at most, 1 subject each
    subjectsFilter = groups
      .slice(0, 5)
      .map((sentenceGroup) => sentenceGroup.subjects.map((subject) => subject.id).slice(0, 1))
      .flat();
  }
